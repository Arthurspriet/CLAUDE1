...
def show_help():
    ...
    table.add_row("", "")
    table.add_row("[bold]HuggingFace Import[/bold]", "")
    table.add_row("/hf-search <query>", "Search HuggingFace for GGUF models")
    table.add_row("/hf-import <repo>", "Download & import GGUF model into Ollama")
    table.add_row("", "")
    table.add_row("[bold]Video Generation[/bold]", "")
    table.add_row("/video-text <prompt>", "Generate video from text prompt")
    table.add_row("/video-image <path>", "Generate video from input image")
    table.add_row("/video-models [type]", "List available video generation models")
...
# ── HuggingFace import rendering ─────────────────────────────────────────────


def show_hf_search_results(results) -> None:
    """Display HuggingFace GGUF search results as a numbered table."""
    if not results:
        console.print("[dim]No GGUF models found. Try a different search query.[/dim]")
        console.print()
        return

    table = Table(title="HuggingFace GGUF Models", border_style="dim")
    table.add_column("#", justify="right", style="bold cyan", width=3)
    table.add_column("Repository", style="bold")
    table.add_column("Downloads", justify="right")
    table.add_column("Likes", justify="right", style="dim")

    for i, r in enumerate(results, 1):
        dl = f"{r.downloads:,}" if r.downloads else "?"
        likes = f"{r.likes}" if r.likes else "-"
        table.add_row(str(i), r.repo_id, dl, likes)

    console.print(table)
    console.print("[dim]Use /hf-import <repo_id> to import a model[/dim]")
    console.print()


def show_hf_gguf_files(repo_id: str, files, budget_mb: int) -> None:
    """Display GGUF files available in a repo with size and quant info."""
    table = Table(title=f"GGUF files in {repo_id}", border_style="dim")
    table.add_column("#", justify="right", style="bold cyan", width=3)
    table.add_column("Quantization", style="bold")
    ...
    for i, f in enumerate(files, 1):
        size_str = f"{f.size_gb:.1f} GB" if f.size_bytes > 0 else "?"
        fits = "[green]yes[/green]" if (f.size_bytes * 1.2) <= budget_bytes else "[red]no[/red]"
        if f.size_bytes == 0:
            fits = "[dim]?[/dim]"
        table.add_row(str(i), f.quant, size_str, fits, f.filename)

    console.print(table)
    console.print()


def show_import_progress(stage: str, message: str) -> None:
    """Display a single import progress line."""
    stage_icons = {
        "search": "[cyan]>>>[/cyan]",
        "detect_vram": "[cyan]>>>[/cyan]",
        "select": "[yellow]>>>[/yellow]",
        "download": "[blue]>>>[/blue]",
        "import": "[magenta]>>>[/magenta]",
        "done": "[green]>>>[/green]",
        "error": "[red]!!![/red]",
    }
...
def show_import_complete(model_name: str) -> None:
    """Display import completion banner."""
    console.print(
        Panel(
            f"[bold green]Model imported successfully![/bold green]\n\n"
            f"Model name: [bold]{model_name}[/bold]\n"
            f"Switch to it: [cyan]/model {model_name}[/cyan]",
            border_style="green",
            padding=(1, 2),
        )
    )
...
def show_vram_info(vram_mb: int, budget_mb: int, description: str) -> None:
    """Display detected VRAM/memory information."""
...
def show_persistent_stats(history: dict) -> None:
    """Display persistent stats history across all sessions."""
    total_sessions = history.get("total_sessions", 0)
    total_tokens = history.get("total_prompt_tokens", 0) + history.get("total_completion_tokens", 0)
    total_requests = history.get("total_requests", 0)

    console.print(f"[bold]All-Time Stats[/bold]")
    ...
    if models:
        table = Table(title="Per-Model Stats", border_style="dim")
        ...
        for name, data in sorted(models.items()):
            table.add_row(
                ...
                f"{data.get('total_prompt_tokens', 0):,}",
                f"{data.get('total_completion_tokens', 0):,}",
    ...
    if sessions:
        recent = sessions[-10:]
        recent_table = Table(title="Recent Sessions", border_style="dim")
        ...
        recent_table.add_column("Requests", justify="right")
