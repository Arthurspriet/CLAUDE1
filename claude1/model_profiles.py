...
- You are an AGENT. When given a task, USE TOOLS to complete it. Do not just describe what to do.
...
- When asked to create/build/implement something, respond with tool calls that create the files and run the code.
- Do NOT write large code blocks in your text output. Use write_file to create files directly.
- After completing the task, summarize what you did in 2-3 sentences. Do not write tutorials.
""".strip()
...
- When asked to create something, use write_file to create it. Never just output code as text.
""".strip()

_NO_OVER_GENERATION = """
- Stop after answering the user's question. Do not add unsolicited suggestions or follow-up ideas.
- Keep text responses under 150 words. Your work shows through tool calls, not prose.
""".strip()
...
PROFILES: dict[str, ModelProfile] = {
    ...
    # Uncensored / abliterated model families (imported from HuggingFace GGUF)
    ...
    "abliterated": ModelProfile(
        family="abliterated",
        display_name="Abliterated (Uncensored)",
        supports_tools=True,
        num_ctx=8192,
        behavioral_rules=_TOOL_CALLING_BEHAVIOR,
    ),
    ...
    "wizardlm-uncensored": ModelProfile(
        family="wizardlm-uncensored",
        display_name="WizardLM Uncensored",
        supports_tools=False,
        num_ctx=8192,
        system_prompt_suffix=_STANDARD_FORMAT_RULES + "\n" + _NO_TOOLS_RULES,
    ),
    "nous-hermes": ModelProfile(
        family="nous-hermes",
        display_name="Nous Hermes",
        supports_tools=True,
        num_ctx=8192,
